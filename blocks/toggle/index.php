...
// Si la imagen va a la derecha, el contenido queda a la izquierda.
...
<section id="<?php echo esc_attr($section_id); ?>" class="<?php echo esc_attr($class_name); ?>">
  <div class="main-section">
    ...
    <div class="image-side">
      <div class="image-stage">
        <?php if ($has_items): ?>
          ...
          ?>
            <div
              class="split-image<?php echo $i === 0 ? ' active' : ''; ?>"
    ...
    <div class="content-side">
      <div class="content-pattern-stage" aria-hidden="true">
        <?php if ($has_items): ?>
          <?php foreach ($items as $i => $row):
            $img        = (isset($row['img_repeater']) && is_array($row['img_repeater'])) ? $row['img_repeater'] : null;
            $pattern_id = $section_id . '-pattern-' . ($i + 1);
            $url        = ($img && !empty($img['url'])) ? esc_url($img['url']) : '';

            if (!$url) continue;
          ?>
            <div
              class="content-pattern<?php echo $i === 0 ? ' active' : ''; ?>"
              id="<?php echo esc_attr($pattern_id); ?>"
      ...
      <div class="content-container" style="<?php echo esc_attr($panel_style); ?>">
        ...
        <div class="accordion-container">
          <?php if ($has_items): ?>
            ...
            ?>
              <div
                class="accordion-item<?php echo $i === 0 ? ' active' : ''; ?>"
                data-image="<?php echo esc_attr($img_id); ?>"
                data-pattern="<?php echo esc_attr($pattern_id); ?>"
              >
  ...
  (function () {
    ...
    function setActiveByIndex(index) {
      ...
      items.forEach(function (item, i) {
        ...
        var patternId = item.getAttribute('data-pattern');

        var image   = imageId ? root.querySelector('#' + CSS.escape(imageId)) : null;
        var pattern = patternId ? root.querySelector('#' + CSS.escape(patternId)) : null;
        ...
        if (i === index) {
          ...
          if (pattern) pattern.classList.add('active');
        } else {
          item.classList.remove('active');
          item.setAttribute('aria-expanded', 'false');
          if (content) content.classList.remove('active');
          if (image) image.classList.remove('active');
          if (pattern) pattern.classList.remove('active');
        }
      });
